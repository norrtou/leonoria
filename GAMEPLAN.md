...
| Phase | Name | Status |
|-------|------|--------|
| 1 | Game spine: GameState + scene flow | **COMPLETE** — verified in-browser 2026-07-04: fresh boot has Continue disabled; new game → party → world gen (seed 12345) → 2 moves advance the clock (8→12h, 12 mi, explored grows) → Save & Quit → reload → Continue restores same seed/position/clock/explored and a byte-identical regenerated map (JSON hash match) |
...
| 5 | Audio director, fog of war, day/night | **BUILT (v1)** — audio-director.js (music/ambience channels, crossfade, autoplay unlock; title theme wired) + CREDITS.md; fog of war (canvas → single SVG image, path cells revealed, soft edges); day/night CSS tint on the overworld. Travel events done (non-combat road happenings: foraging, caches, shrines, weather, flavor). Music/ambience assets done & verified in-browser: 11 Kevin MacLeod tracks (CC-BY, assets/sounds/music/, re-encoded VBR) + 3 OGA ambience loops (assets/sounds/ambience/, ogg for gapless looping), all in CREDITS.md. Per-biome/scene tracks wired: `AudioDirector.playScene(scene, biomeId)` dispatched from `Scenes.show` (title/party/world→theme, overworld→BIOME_MUSIC/BIOME_AMBIENCE per 10 biomes, battle→Five Armies, dungeon→Ossuary+cave drips) and from the settlement open/close call sites (tavern music). Menu gets a 🔊 Sound toggle + per-channel Music/Ambience/Effects sliders (persisted in localStorage `leonoria_audio_prefs`; battle-map SFX gain follows the Effects slider). Combat SFX expanded & verified in-browser: spell cast/impact sounds per damage type (SPELL_CAST_SND/SPELL_HIT_SND + castSnd() in battle-map.js — fire/lightning/ice/smite/arcane/shadow/nature, generic spellcast fallback; previously everything used the fire sound), monster attack voices per archetype (wolf growl, brute roar, synthesized serpent hiss, undead moan), monster death sound, battle-start horn stinger, per-weapon melee hit sounds (sword/stab kept; axe chop, blunt thud, spear thrust, whip crack, shield bash — skill attacks via SKILL_ATTACK_MAP snd keys, equipped loot weapons via WEAPON_CAT_SND category map). Sources: RPG Sound Pack (artisticdude, CC0), Ice Spells (bart, CC0), Magic Smite (spookymodem, CC-BY), Massive Horn (Kat, CC-BY) — all in CREDITS.md. **Phase complete** |
