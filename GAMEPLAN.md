...
| 2 | Encounter bridge: overworld → battle → results | **BUILT (v1)** — biome battlefield palettes done (BIOME_PALETTES in battle-map.js recolors ground/shrubs/flowers/rocks per biome; tree foliage stays green — known compromise). Loot drops done (loot.js: trophies + weapons from the real weapon×material×quality system, sellable at traders). Remaining: per-monster sprites/stats (rosters map onto goblin/goblin_archer archetypes), flee option |
| 3 | Settlements as hubs: shop, inn, rest, rumors | **BUILT (v1)** — inn rest, provisions, temple healing, persistent wounds, food per travel day, rumors→quests, trader (sell loot; buy armor/oils/charms upgrades that raise HP/damage/dodge in battle via gearBonus). Remaining: reputation, per-member weapon equipping, economy.json price modulation |
